...
with TestClient(app) as client:
    ...
    r = client.post(f"/api/placement/{pid}/start")
    sid = r.json()["session_id"]
    topics_seen, rounds = set(), 0
    while True:
        nxt = client.get(f"/api/placement/{sid}/next").json()
        if nxt.get("complete"):
            break
        rounds += 1
        ...
        sub = client.post(f"/api/placement/{sid}/submit", json={
    ...
    r = client.get(f"/api/placement/{sid}/summary").json()
    ...
    attempts = client.get(f"/api/placement/{sid}/summary").json()
    # grab any answered question id via quiz summary route not available; reuse last nxt
    r = client.post(f"/api/placement/{sid}/submit", json={
    ...
    r2 = client.post(f"/api/placement/{pid}/start")
    sid2 = r2.json()["session_id"]
    nxt2 = client.get(f"/api/placement/{sid2}/next").json()
    client.post(f"/api/placement/{sid2}/submit", json={
        "question_id": nxt2["question_id"], "selected_answer": nxt2["options"][0]})
    dup = client.post(f"/api/placement/{sid2}/submit", json={
    ...
    r = client.post(f"/api/quiz/{pid}/start", json={"topic_filter": "Mechanics",
                                               "num_questions": 5})
    qsid = r.json()["session_id"]
    theta_before = client.get(f"/api/players/{pid}").json()["theta_overall"]
    rounds = 0
    while True:
        ...
        db.close()
        sub = client.post(f"/api/quiz/{qsid}/submit", json={
    ...
    r = client.get(f"/api/quiz/{qsid}/summary").json()
    ...
    theta_after = client.get(f"/api/players/{pid}").json()["theta_overall"]
    ...
    pr = client.get(f"/api/players/{pid}").json()
    ...
    r = client.post(f"/api/quiz/{pid}/start", json={"num_questions": 1})
    qsid = r.json()["session_id"]
    nxt = client.get(f"/api/quiz/{qsid}/next").json()
