...
with TestClient(app) as client:
    pid = client.post("/players/create", json={"name": "TestKid"}).json()["player_id"]

    print("1) run a 4-question quiz: 2 right, 2 deliberately wrong")
    qsid = client.post(f"/quiz/{pid}/start", json={"num_questions": 4}).json()["session_id"]
    ...
    target = wrong_qids[0]
    r = client.post(f"/reports/{qsid}/failure_test/{target}")
    ...
    if ft.get("fallback"):
        ...
        again = client.post(f"/reports/{qsid}/failure_test/{target}").json()
        ...
        for p, resp_text in zip(probes, canned):
            rr = client.post(
                f"/reports/{qsid}/failure_test/{target}/respond",
        ...
        check("respond after complete -> 409",
              client.post(f"/reports/{qsid}/failure_test/{target}/respond",
    ...
    print("\n7) guard rails")
    ok_q = [w["question_id"] for w in client.get(f"/reports/{qsid}").json()["wrong_answers"]]
    r = client.post(f"/reports/{qsid}/failure_test/physics_kinematics_nonexistent")
    check("failure test on unattempted question -> 404", r.status_code == 404)
    # a correctly-answered question can't be failure-tested
    all_attempted = client.get(f"/reports/{qsid}").json()
    correct_qid = None
    summary = client.get(f"/quiz/{qsid}/summary").json()
    correct_qid = next(q["question_id"] for q in summary["questions"] if q["was_correct"])
    r = client.post(f"/reports/{qsid}/failure_test/{correct_qid}")
